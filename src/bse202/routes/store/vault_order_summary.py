...
def vault_order_summary(purchase_id: str):
    ...
    try:
        ...
        items = list(map(
            lambda tup: {"game_id": tup[0], "purchased_at": datetime.fromtimestamp(tup[1]).strftime("%d/%m/%Y - %H:%M"), "title": tup[2], "price": tup[3], "image_id": tup[4]},
    ...
    return render_template("views/vault_order_summary.html", items = items, total_price=total_price)
